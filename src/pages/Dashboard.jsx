  ...
  const loadData = async () => {
    ...
    try {
      const [dashData, actData] = await Promise.all([
        api.getDashboard(),
        isAdmin ? api.getActivity({ limit: 8 }) : Promise.resolve({ logs: [] }),
      ]);
      setStats(dashData);
    ...
    <div className="page-enter">
      <div style={{ marginBottom: 28 }}>
        ...
        <p style={{ color: "var(--text-muted)", fontSize: 14, marginTop: 4 }}>
          {isAdmin ? "Overview of your inventory system" : "Overview of your active assignments"}
        </p>
      </div>
      ...
      <div style={{ display: "flex", gap: 16, flexWrap: "wrap", marginBottom: 28 }}>
        {isAdmin ? (
          ...
          <StatCard
            label="My Active Assignments" value={stats.userActiveAssignments || 0}
            color="var(--accent)"
            icon={<Icon d="assign" size={20} />}
          />
        )}
      ...
      {/* Overdue Warning */}
      {isAdmin && stats.overdueAssignments > 0 && (
        <div style={{
